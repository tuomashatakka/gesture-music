...
export class GestureDebouncer {
  ...
  update(raw: GlyphType): GlyphType {
    ...
    if (this.history.length < 4) return raw;
    ...
    for (const [g, c] of Object.entries(counts) as [GlyphType, number][]) {
      if (g !== 'unknown' && c > bestCount) { best = g; bestCount = c; }
    }

    if (bestCount / n >= SWITCH_THRESH) this._stable = best;
    return this._stable;
  }

  get stable(): GlyphType { return this._stable; }

  reset(): void { this.history = []; this._stable = 'unknown'; }
}
